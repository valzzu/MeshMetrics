class PositionUtil {
  ...
  static setPositionPrecision(latitudeOrLongitudeInteger, precision) {
    ...
    if (precision > 0 && precision < 32) {
      ...
      latitudeOrLongitudeInteger =
        latitudeOrLongitudeInteger & (0xffffffff << (32 - precision));
   ...
   * @returns {*|string|null}
   */
  static truncateDecimalPlaces(
    latitudeOrLongitudeString,
    ...
    var [leftOfDecimalPoint, rightOfDecimalPoint] =
      latitudeOrLongitudeString.split(".");

    // check if decimal places available
    if (rightOfDecimalPoint != null) {
      // truncate decimal places to desired length
      rightOfDecimalPoint = rightOfDecimalPoint.substring(
        0,
        numberOfDecimalPlaces
      );
